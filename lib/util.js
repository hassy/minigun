...
>>>>>>> wip: Refactor implementation of ensure
    });
...
function checkEnsureConditions(script, report) {
  let result = {errors: [], warnings: []};

  if (typeof script.config.ensure !== 'object') {
    return result;
  }
  ...
  _.each(script.config.ensure, function(max, k) {
    ...
    if (latency[bucket]) {
      if (latency[bucket] > max) {
        result.errors.push({ type: 'latency', message: `${bucket} latency is higher than ${max}`});
      }
    }
  });
  ...
  if (typeof script.config.ensure.maxErrorRate !== 'undefined') {
    ...
    if (failRate > script.config.ensure.maxErrorRate) {
      result.errors.push({type: 'maxErrorRate', message: `ensure condition failed: ensure.maxErrorRate <= ${script.config.ensure.maxErrorRate}`});
    }
  }
