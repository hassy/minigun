...
module.exports.getConfig = function(callback) {
  let commandConfig = {
    ...
    options: [
      ...
      [
        '--probe',
        '[Experimental] Set to probe mode'
      ],
  ...
  async.waterfall(
    ...
    function done(err, script) {
      ...
      runner.events.on('done', function(allStats) {
        ...
        if (options.probe) {
          script.config.ensure = script.config.ensure || {};
          script.config.ensure.maxErrorRate = 0;
        }

        if (script.config.ensure) {
          const result = checkEnsureConditions(script, report);
          if (result.errors.length > 0) {
            if (!options.quiet) {
              let ensureErrors = result.errors;
              // If --probe is set, we don't need to display the maxErrorRate error
              if (options.probe) {
                ensureErrors = ensureErrors.filter(e => e.type !== 'maxErrorRate');
              }
              console.log(chalk.red(ensureErrors.map(e => `- ${e.message}`).join('\n')));
            }
            process.exit(1);
          }
        }
