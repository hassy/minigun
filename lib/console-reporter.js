...
ConsoleReporter.prototype.phaseStarted = function phaseStarted(phase) {
  ...
  this.toggleSpinner();
  console.log(
    'Phase started: %s (index: %s, duration: %ss) %s\n',
    chalk.green(phase.name ? phase.name : 'unnamed'),
    phase.index,
...
ConsoleReporter.prototype.printReport = function printReport(report, opts) {
  ...
  if (opts.printPeriod !== false) {
    ...
    const txt = 'Metrics for period to: ' + moment(new Date(report.period + 10 * 1000)).format('HH:mm:ss(ZZ)');
