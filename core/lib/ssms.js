...
class SSMS extends EventEmitter {
  ...
  // TODO: Consider making this a static method for caller's convenience, e.g.
  // for serializing period data in a 'period' event handler
  serializePeriodJSON(periodTs) {
    // TODO: Add ability to include arbitrary metadata e.g. worker IDs
    const result = {};
    result[periodTs] = {
      counters: this._aggregatedCounters[periodTs] || {},
      histograms: {}
    };

    const ph = this._aggregatedHistograms[periodTs];
    if (ph) {
      for(const n of Object.keys(ph)) {
        const h = ph[n];
        const buf = h.toProto();
        result[periodTs].histograms[n] = buf;
      }
    }
  ...
  static deserializePeriodJSON(periodData) {
    const result = {};
    const obj = parse(periodData);
    const ts = Object.keys(obj)[0];
    result[ts] = {
      histograms: {},
      counters: Object.assign({}, obj.counters)
    };

    for (const [name, value] of Object.entries(obj[ts].counters)) {
      result[ts].counters[name] = value;
    }

    for (const [name, buf] of Object.entries(obj[ts].histograms)) {
      // FIXME: currently summary stats are lost, see:
      // https://github.com/DataDog/sketches-js/blob/master/src/ddsketch/DDSketch.ts#L200
      result[ts].histograms[name] = DDSketch.fromProto(buf);
    }
    return result;
  }
  ...
  _maybeEmitMostRecentPeriod() {
    ...
    if (p && p != this._lastPeriod) {
      this.emit('period', p, this.getMetrics(p)); // Measurements in period p have been aggregated
      this._lastPeriod = p;
    }
  }
}
